...
const ThankYouPage = async ({ searchParams }: PageProps) => {
  ...
  const { docs: orders } = await payload.find({
    collection: "orders",
    depth: 2, // this will give us the actual user instead of just the user id
    where: {
      id: {
        equals: orderId,
      },
  ...
  // make sure this user is the same one that bought the order
  // if order.user is a string then its a userid, so just return order.user,
  // otherwise its the user object so return order.user.id
  // ** we know it will be a user object because we set the depth to 2 but typescript doesnt know that **
  const orderUserId =
    typeof order.user === "string" ? order.user : order.user.id;

  if (orderUserId !== user?.id) {
    // this user is not logged in or authorized
